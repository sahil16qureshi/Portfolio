      ...
      <div className="section-shell">
        ...
        <div className="relative">
          ...
          {experience.map((item, index) => (
            ...
            >
              <div className="absolute left-0 top-6 hidden h-10 w-10 items-center justify-center rounded-full border border-line bg-panel text-redSoft md:flex">
                <BriefcaseBusiness className="h-5 w-5" />
              </div>
              <article className="glass-card p-6">
                <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                  <div>
                    <p className="text-sm font-semibold uppercase tracking-[0.18em] text-redSoft">
                      {item.company}
                    </p>
                ...
                <ul className="mt-6 grid gap-3 text-sm leading-6 text-slate-300">
                  {item.bullets.map((bullet) => (
                    <li key={bullet} className="flex gap-3">
                      <span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-redSoft" />
                      <span>{bullet}</span>
                    </li>
                  ))}
                </ul>
